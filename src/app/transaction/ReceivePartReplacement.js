...
function ReceivePartReplacement() {
  const [currentPage, setCurrentPage] = useState(1);
  const [sizePerPage, setSizePerPage] = useState(5);
  const [show, setShow] = useState(false);
  const [datas, setDatas] = useState(JSON.parse(localStorage.getItem('dataRc')).filter((el) => el.status === 'GIVE PART' && el.no_surat_jalan !== ""));
      ...
      <div className="container d-flex p-md-0">
        <div className="az-content-body pd-lg-l-40 d-flex flex-column">
          <div className="az-content-breadcrumb">
            ...
            <span>RECEIVE PART REPLACEMENT</span>
          </div>
          <h2 className="az-content-title">Receive Part Replacement</h2>
...
export default ReceivePartReplacement;
